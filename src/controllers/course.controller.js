...
const getCourses = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'An ERROR has occurred when getting all courses.' }, error);
  }
}
...
const getCourse = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'An ERROR has occurred when getting the course.' }, error);
  }
};
...
const addCourse = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: `An ERROR has occured when creating the course: ${error}` });
  }
};
...
const updateCourse = async (req, res) => {
  try {
    ...
    res.status(200).send('Registry updated successfully.');
  } catch (error) {
